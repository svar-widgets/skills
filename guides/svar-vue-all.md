...
Use when building, configuring, styling, or modifying SVAR Vue Filemanager / @svar-ui/vue-filemanager components, menus, themes, store actions, or backend data provider integration.
...
- `Willow`, and `WillowDark` wrap matching `@svar-ui/vue-core` themes and add filemanager CSS variables.
- Theme components accept `fonts?: boolean` and optional Vue children.
- `Filemanager` has no `children` slot, `class`, `style`, or `css` passthrough prop.
...
- `ref="api"` exposes the same public methods: `exec`, `on`, `intercept`, `detach`, `setNext`, `getState`, `getReactiveState`, `getStores`, `getFile`, and `serialize`.
...
- Menu popups come from `@svar-ui/vue-menu` and use `.wx-menu`, `.wx-option`, `.wx-separator`, `.wx-subtext`, and related menu hooks; the popup is portaled outside the filemanager layout.
...
```vue
<script setup>
import { Filemanager, Willow } from "@svar-ui/vue-filemanager";
...
```vue
<script setup>
import { Filemanager, getMenuOptions } from "@svar-ui/vue-filemanager";
...
<template>
  <Filemanager :data="data" :drive="drive" :init="init" :menuOptions="menuOptions" />
</template>
```
...
<template>
  <Filemanager :init="init" :data="data" :drive="drive" :onrequestdata="loadData" />
</template>
```
...
<template>
  <Filemanager :data="data" :drive="drive" :icons="icons" :previews="previews" :extraInfo="extraInfo" preview />
</template>
```
...
- `Filemanager.vue` stores all unknown props in `restProps` for generated action callbacks; unknown props are not spread onto DOM.
...
- `registerMenuItem("upload", UploadButton)` runs inside `Sidebar.vue`; a custom `"upload"` add item only works through the built-in sidebar add menu.
